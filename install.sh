...
y | ./snell-server
echo -e "\nobfs = off" >> ./snell-server.conf
systemctl start snell
systemctl enable snell			# start service

echo
echo "Copy the following line to surge"			# print profile
echo "$(curl -s ipinfo.io/city) = snell, $(curl -s ipinfo.io/ip), $(cat snell-server.conf | grep -i listen | cut --delimiter=':' -f2),psk=$(grep 'psk' snell-server.conf | cut -d= -f2 | tr -d ' '), obfs = off, version=4, tfo=true"
